...
### Add product page - awaiting aaron

# Send HTML, process form await aaron
@app.route('/add_product', methods=['GET', 'POST'])
def add_product():
	if (request.method == 'GET'):
		res = make_response(render_template('add_product.html'))
		res.set_cookie('id', 'a4')
...
def login_student():
	if (request.method == 'GET'):
		...
		if (len(rows) == 0):
			...
			res.set_cookie('id', rows[0][0])
			res.set_cookie('type', 'student')
			print('login student ok', uname, pword)
		else:
			res = make_response('Account already exists!')
			print('login student taken', uname, pword)
		return res
